...
public class BoardDAOMyBatis implements BoardDAO {
	...
	public void missionJoin(Map<Object, Object> map) {
		...
		if (tmp.getMembers() == null) {
			map.put("members", map.get("id"));
		} else {
			map.put("members", tmp.getMembers() + " " + map.get("id"));
		}
		if (tmp2.getBoards() == null) {
			map.put("boards", map.get("seq"));
		} else {
			map.put("boards", tmp2.getBoards() + " " + map.get("seq"));
		}
